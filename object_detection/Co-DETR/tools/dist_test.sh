...
for hf_dataset in "${hf_datasets[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=3 PYTHONPATH="$(dirname $0)/..":$PYTHONPATH \
  python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port=$PORT \
    $(dirname "$0")/test.py $CONFIG $CHECKPOINT --launcher pytorch ${@:4} --format-only --options "jsonfile_prefix=./det_output/results_" \
    --hf-dataset $hf_dataset --cache-dir $cache_dir --custom-dataset-save-path $custom_dataset_save_path
done