...
class OVSegVisualizer(Visualizer):
    ...
    def draw_sem_seg(self, sem_seg, area_threshold=None, alpha=0.8):
        ...
        for label in filter(lambda l: l < len(class_names), labels):
            ...
            self.binary_masks.append(binary_mask)
...
def mask_to_boxes(mask):
    mask = np.array(mask)
    bboxes = []
    for obj in range(mask.shape[0]):  # Loop through each object
        obj_mask = mask[obj]  # Get the mask for the current object
        coords = np.argwhere(obj_mask > 0.5)
        if coords.shape[0] > 0:  # Check if there are any pixels
            x_min, y_min = coords.min(axis=0)  # Get min x and y coordinates
            x_max, y_max = coords.max(axis=0)  # Get max x and y coordinates
            bboxes.append(np.array([x_min, y_min, x_max, y_max]).tolist())
    return bboxes
class VisualizationDemo(object):
    ...
    def run_on_image(self, image, class_names):
        ...
        if "sem_seg" in predictions:
            ...
            predictions["labels"] = visualizer.text_labels
            predictions["masks"] = visualizer.binary_masks
            predictions["bboxes"] = mask_to_boxes(visualizer.binary_masks)
